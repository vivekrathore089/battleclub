...
const WithdrawalManagementClient = () => {
    const { toast } = useToast();
    const { withdrawalRequests, processWithdrawalRequest, addTransaction } = useRequests();
    const { updateUserWallet } = useAuth();
    const [action, setAction] = useState<{ type: 'approve' | 'reject'; request: WithdrawalRequest | null }>({ type: 'approve', request: null });

    const handleAction = () => {
        if (!action.request) return;

        if (action.type === 'approve') {
             processWithdrawalRequest(action.request.id, (userId, amount) => {
                updateUserWallet(userId, amount, 'winnings', 'remove');
                addTransaction({
                    type: 'Withdrawal',
                    amount: -amount,
                    description: `To: ${action.request?.upiId}`,
                });
            });
        } else {
             processWithdrawalRequest(action.request.id, () => {});
        }

        toast({
            title: `Request ${action.type === 'approve' ? 'Approved' : 'Rejected'}`,
            description: `The withdrawal request for ${action.request.userName} has been processed.`,
        });
                    ...
                    <TableBody>
                        {withdrawalRequests.length > 0 ? withdrawalRequests.map((request) => (
                            <TableRow key={request.id}>
                                ...
                                <TableCell className="text-muted-foreground">{request.upiId}</TableCell>
                ...
                <AlertDialogContent className="bg-card border-secondary">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            You are about to {action.type} a withdrawal request of <span className="font-bold text-primary-foreground">₹{action.request?.amount.toFixed(2)}</span> for <span className="font-bold text-primary-foreground">{action.request?.userName}</span>. This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
...
export default WithdrawalManagementClient;
